...
def get_api_key() -> str:
    # 1) Streamlit Cloud의 Secrets 우선 사용 (배포 환경)
    try:
        key = st.secrets.get("KAKAO_REST_API_KEY", "")
        if key:
            return str(key).strip()
    except (FileNotFoundError, st.errors.StreamlitAPIException, Exception):
        pass
    # 2) 로컬 .env 파일 fallback
...
def kakao_search_cached(query: str, api_key: str, page: int = 1) -> Dict:
    ...
    if response.status_code == 401:
        raise RuntimeError("카카오 REST API 키 인증 실패: .env의 REST API 키를 확인하세요.")
...
if submitted:
    ...
    api_key = get_api_key()
    if not api_key:
        st.session_state.last_error = "KAKAO_REST_API_KEY가 설정되지 않았습니다. 로컬 실행 시에는 .env 파일을, Streamlit Cloud 배포 시에는 앱 Settings → Secrets에 키를 추가하세요."
    elif not moods:
