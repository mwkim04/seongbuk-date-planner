...
APP_VERSION = "final_bar_keyword_pink_fixed"
PINK = "#ff4f9a"
HOT_PINK = "#ff2f86"
LIGHT_PINK = "#fff2f7"
BORDER = "#ffd0e2"
...
/* 슬라이더 빨강 제거 → 핑크 (색 문자열에 'rgb(255'가 들어간 채움 요소 전부 강제; 회색 rail은 rgba(151..)이라 영향 없음) */
...
/* 구조 기반 강제: 슬라이더 thumb(손잡이)를 핑크로 (인라인이 아니라 클래스로 칠해진 경우 대비) */
[data-baseweb="slider"] [role="slider"],
section[data-testid="stSidebar"] [data-baseweb="slider"] [role="slider"] {{
  background:#ff4f9a !important; background-color:#ff4f9a !important; border-color:#ff4f9a !important;
}}
.hero {{
...
/* 체크된 체크박스 빨강 제거 → 핑크 (색 문자열 + 구조 기반 강제) */
[data-testid="stCheckbox"] [style*="rgb(255"],
[data-testid="stCheckbox"] [style*="#ff4b4b"],
[data-testid="stCheckbox"] [style*="#FF4B4B"],
[data-baseweb="checkbox"] [style*="rgb(255"],
...
/* 구조 기반: 체크된 박스(보통 label 안 첫 번째 박스 요소)를 핑크로 (클래스로 칠해진 경우 대비) */
[data-baseweb="checkbox"] [data-checked="true"],
[data-baseweb="checkbox"] span[aria-checked="true"],
section[data-testid="stSidebar"] [data-testid="stCheckbox"] label > span:first-child[aria-checked="true"] {{
  background:#ff4f9a !important; background-color:#ff4f9a !important; border-color:#ff4f9a !important;
}}
...
/* 빨간 기본 포인트(슬라이더/체크박스)만 핑크로 강제 덮어쓰기 */
:root,
html,
body,
.stApp,
...
section[data-testid="stSidebar"] [data-testid="stCheckbox"] {{
  --primary-color:#ff4f9a !important;
  --primary-color-rgb:255,79,154 !important;
  --primary-color-hover:#ff2f86 !important;
  --primary-color-active:#ff2f86 !important;
  accent-color:#ff4f9a !important;
}}

/* 사이드바 슬라이더: 빨간 채움/손잡이만 핑크로 */
section[data-testid="stSidebar"] [data-testid="stSlider"] [data-baseweb="slider"] [role="slider"],
section[data-testid="stSidebar"] [data-testid="stSlider"] [data-baseweb="slider"] [aria-valuenow],
section[data-testid="stSidebar"] [data-testid="stSlider"] [data-baseweb="slider"] [class*="thumb"],
section[data-testid="stSidebar"] [data-testid="stSlider"] [data-baseweb="slider"] [class*="Thumb"] {{
  background:#ff4f9a !important;
  background-color:#ff4f9a !important;
  border-color:#ff4f9a !important;
...
section[data-testid="stSidebar"] [data-testid="stSlider"] [data-baseweb="slider"] [style*="rgb(255, 75, 75)"],
section[data-testid="stSidebar"] [data-testid="stSlider"] [data-baseweb="slider"] [style*="rgb(255,75,75)"],
section[data-testid="stSidebar"] [data-testid="stSlider"] [data-baseweb="slider"] [style*="#ff4b4b"],
section[data-testid="stSidebar"] [data-testid="stSlider"] [data-baseweb="slider"] [style*="#FF4B4B"] {{
  background:#ff4f9a !important;
  background-color:#ff4f9a !important;
  border-color:#ff4f9a !important;
...
/* 사이드바 체크박스: 체크된 박스만 핑크로 */
section[data-testid="stSidebar"] [data-testid="stCheckbox"] input[type="checkbox"]:checked,
section[data-testid="stSidebar"] [data-testid="stCheckbox"] input[type="checkbox"]:checked + div,
section[data-testid="stSidebar"] [data-testid="stCheckbox"] label:has(input[type="checkbox"]:checked) > span:first-child,
section[data-testid="stSidebar"] [data-testid="stCheckbox"] [aria-checked="true"],
section[data-testid="stSidebar"] [data-testid="stCheckbox"] [data-checked="true"] {{
  background:#ff4f9a !important;
  background-color:#ff4f9a !important;
  border-color:#ff4f9a !important;
}}

section[data-testid="stSidebar"] [data-testid="stCheckbox"] [style*="rgb(255, 75, 75)"],
section[data-testid="stSidebar"] [data-testid="stCheckbox"] [style*="rgb(255,75,75)"],
section[data-testid="stSidebar"] [data-testid="stCheckbox"] [style*="#ff4b4b"],
section[data-testid="stSidebar"] [data-testid="stCheckbox"] [style*="#FF4B4B"] {{
  background:#ff4f9a !important;
  background-color:#ff4f9a !important;
  border-color:#ff4f9a !important;
...
# 프로세스 전역 캐시(스레드 안전). 동일 쿼리는 한 번만 호출하고, 재실행/타 사용자와도 공유한다.
_RAW_CACHE: Dict[Tuple[str, int], Dict] = {}
_RAW_CACHE_LOCK = threading.Lock()
_KAKAO_MAX_WORKERS = 16  # 병렬 호출 동시 처리 수 (카카오 초당 한도 내에서 콜드 수집 시간을 단축)


def _kakao_fetch(query: str, api_key: str, page: int = 1) -> Dict:
    """실제 네트워크 호출. 429(한도 초과)는 짧은 백오프로 재시도."""
    ...
    for attempt in range(3):
        ...
        if response.status_code == 401:
            raise RuntimeError("카카오 REST API 키 인증 실패: REST API 키를 확인하세요.")
    ...
    if last_exc is not None:
        raise RuntimeError(f"카카오 API 호출 실패: {last_exc}")
    raise RuntimeError("카카오 API 호출 한도 초과: 잠시 후 다시 시도하세요.")


def kakao_search_cached(query: str, api_key: str, page: int = 1) -> Dict:
    """캐시 우선 조회. 미스면 네트워크 호출 후 캐시에 저장."""
    key = (query, page)
    with _RAW_CACHE_LOCK:
        cached = _RAW_CACHE.get(key)
    if cached is not None:
        return cached
    data = _kakao_fetch(query, api_key, page)
    with _RAW_CACHE_LOCK:
        _RAW_CACHE[key] = data
    return data


def prefetch_queries(queries: List[str], api_key: str) -> None:
    """주어진 쿼리들의 1~2페이지를 병렬로 미리 받아 캐시를 채운다.

    - 1페이지를 모두 병렬 수집한 뒤, is_end가 아닌 쿼리만 2페이지를 병렬 수집한다.
    - 결과는 _RAW_CACHE에 저장되므로 이후 처리 루프는 캐시 적중으로 즉시 진행된다.
    """
    uniq = list(dict.fromkeys(q for q in queries if q))

    def _warm(query: str, page: int):
        kakao_search_cached(query, api_key, page)
        with _RAW_CACHE_LOCK:
            return _RAW_CACHE.get((query, page), {})

    # 1페이지 병렬 수집
    need_page2: List[str] = []
    with ThreadPoolExecutor(max_workers=_KAKAO_MAX_WORKERS) as ex:
        futures = {ex.submit(_warm, q, 1): q for q in uniq}
        for fut in as_completed(futures):
            q = futures[fut]
            data = fut.result()  # 401 등 치명적 오류는 그대로 전파
    ...
    if need_page2:
        with ThreadPoolExecutor(max_workers=_KAKAO_MAX_WORKERS) as ex:
            futures = {ex.submit(_warm, q, 2): q for q in need_page2}
            for fut in as_completed(futures):
                fut.result()
...
def mood_tags_for(name: str, category_name: str, place_type: str) -> List[str]:
    ...
    if place_type in ["카페", "산책", "포토부스"]:
        tags.update(["감성", "조용한"])
    if place_type in ["활동", "술집", "산책", "포토부스"]:
        tags.update(["활동적"])
...
def place_matches_moods(place: Dict, moods: List[str]) -> bool:
    """장소의 분위기 태그가 선택한 분위기 중 하나라도 포함하면 True."""
    if not moods:
        return True
    tags = {t.strip() for t in str(place.get("mood_tags", "")).split(",") if t.strip()}
    return any(m in tags for m in moods)
...
def collect_places(region: str, include_bar: bool, api_key: str, include_terms: List[str] | None = None) -> pd.DataFrame:
    ...
    region_terms = REGION_KEYWORDS.get(region, [region])[:2]

    # 처리 루프에서 사용할 모든 쿼리를 미리 만들어 병렬로 캐시를 채운다(순차 호출 → 병렬 호출).
    planned_queries: List[str] = []
    for requested_group in groups:
        for region_term in region_terms:
            for keyword in SEARCH_KEYWORDS[requested_group]:
                planned_queries.append(f"{region_term} {keyword}")
    for term in include_terms:
        for region_term in region_terms:
            planned_queries.append(f"{region_term} {term}")
    prefetch_queries(planned_queries, api_key)
...
def add_scores(df: pd.DataFrame, moods: List[str], include_terms: List[str] | None = None) -> pd.DataFrame:
    ...
    # 빈 결과가 들어와도 안전하게 빈 프레임을 반환(호출부에서 별도 처리).
    if df.empty:
...
def weighted_pick_from_candidates(cand: List[Dict]) -> Dict | None:
    """상위 후보 중에서 무작위로 선택해 같은 조건에서도 코스가 반복되지 않도록 함.

    pandas 의존을 제거한 순수 파이썬 구현. rank 내림차순 정렬 후 상위 14개에서
    가중 무작위 1개를 뽑는다(가중치 = 순위가 높을수록 큼).
    """
    if not cand:
        return None
    ordered = sorted(cand, key=lambda r: r["rank"], reverse=True)[:min(14, len(cand))]
    weights = [max(1.0, len(ordered) - idx) for idx in range(len(ordered))]
    return dict(random.choices(ordered, weights=weights, k=1)[0])


_LEISURE_TYPES = ("포토부스", "활동", "산책")


def build_place_index(df: pd.DataFrame) -> Dict[str, List[Dict]]:
    """DataFrame을 타입별 dict 리스트로 1회만 변환해 이후 선택을 빠르게 한다."""
    index: Dict[str, List[Dict]] = {}
    for rec in df.to_dict("records"):
        index.setdefault(rec["type"], []).append(rec)
    return index


def _slot_candidates(index: Dict[str, List[Dict]], slot: str) -> List[Dict]:
    if slot == "여가":
        out: List[Dict] = []
        for t in _LEISURE_TYPES:
            out.extend(index.get(t, []))
        return out
    return list(index.get(slot, []))
...
def choose_candidate_bulletproof(
    ...
    prefer_moods: bool = False,
) -> Dict | None:
    include_terms = include_terms or []
    moods = moods or []
    cand = _slot_candidates(index, slot)

    # 같은 여가 유형이 반복되는 것을 줄임. 단 후보가 완전히 없으면 같은 유형 재사용보다 해당 코스는 포기.
    if slot == "여가" and strict:
        narrowed = [c for c in cand if c["type"] not in used_leisure_types]
        if narrowed:
            cand = narrowed

    # 이름 중복은 피함. 후보가 부족하더라도 전혀 다른 분류로 억지 대체하지 않음.
    cand = [c for c in cand if c["name"] not in used_names]
    if not cand:
        return None

    # 반드시 포함 키워드가 아직 충족되지 않았고 현재 슬롯에서 매칭 후보가 있으면 그 후보를 우선 선택.
    if prefer_terms and include_terms:
        matched = [c for c in cand if row_matches_terms(c, include_terms)]
        if matched:
            cand = matched

    # 분위기 매칭 장소가 아직 부족하면, 현재 슬롯에서 분위기에 맞는 후보를 우선 선택.
    if prefer_moods and moods:
        mood_matched = [c for c in cand if place_matches_moods(c, moods)]
        if mood_matched:
            cand = mood_matched

    # rank 계산용 사본 생성(원본 index의 dict를 오염시키지 않도록)
    cand = [dict(c) for c in cand]
    if prev:
        plat, plon = prev["lat"], prev["lon"]
        for c in cand:
            c["near"] = haversine_km((plat, plon), (c["lat"], c["lon"]))
    else:
        for c in cand:
            c["near"] = 0.0

    for c in cand:
        c["rank"] = c["score"] * 2.2 - c["near"] * 0.75 + random.random() * 2.8
...
def _route_from_sequence(
    ...
    include_terms = include_terms or []
    moods = moods or []
    # 분위기를 선택하면 코스에 분위기 매칭 장소를 최소 2개(코스가 더 짧으면 그 수만큼) 포함.
    min_mood = min(2, len(seq)) if moods else 0
    ...
    for slot in seq:
        ...
        mood_so_far = sum(1 for p in places if place_matches_moods(p, moods)) if moods else 0
        need_mood = bool(moods) and (mood_so_far < min_mood)
    ...
    # 분위기 최소 포함 수 미달이면 이 코스는 채택하지 않음.
    if moods:
        mood_count = sum(1 for p in places if place_matches_moods(p, moods))
        if mood_count < min(2, len(places)):
            return None
...
def build_routes(df: pd.DataFrame, budget: int, mode: str, include_bar: bool, place_count: int, max_distance: float, allow_over_budget: bool, include_terms: List[str] | None = None, moods: List[str] | None = None) -> List[Dict]:
    """요청 조건으로 최대 5개 추천안을 빠르게 생성. 술집 체크 시 오래 멈추는 현상을 방지."""
    include_terms = include_terms or []
    moods = moods or []
    # DataFrame을 타입별 dict 인덱스로 1회만 변환(이후 모든 선택은 순수 파이썬으로 처리).
    index = build_place_index(df)
    all_places = [p for plist in index.values() for p in plist]

    if include_terms and not any(row_matches_terms(p, include_terms) for p in all_places):
        raise RuntimeError(f"반드시 포함할 키워드({', '.join(include_terms)})와 일치하는 장소를 찾지 못했습니다. 키워드를 바꾸거나 지역을 넓혀보세요.")

    # 분위기 매칭 장소가 충분히 있을 때만 "최소 2개 포함"을 강제(없는 분위기를 요구해 빈 결과가 되는 것 방지).
    mood_available = sum(1 for p in all_places if place_matches_moods(p, moods)) if moods else 0
    eff_moods = moods if (moods and mood_available >= 2) else []
    ...
    for strict in [True, False]:
        for seq in templates:
            for _ in range(80):
                if len(routes) >= 5:
                    return routes[:5]
                add_route(_route_from_sequence(index, seq, budget, mode, max_distance, allow_over_budget, include_bar, strict, include_terms, eff_moods))
    ...
    if len(routes) < 5 and allow_over_budget:
        for seq in templates:
            for _ in range(60):
                if len(routes) >= 5:
                    return routes[:5]
                add_route(_route_from_sequence(index, seq, budget, mode, max_distance * 1.25, True, include_bar, False, include_terms, eff_moods))

    # 최종 안전망: 분위기 최소 포함 조건 때문에 코스를 하나도 못 만든 경우, 분위기 강제를 풀고 한 번 더 시도.
    if not routes and eff_moods:
        for strict in [True, False]:
            for seq in templates:
                for _ in range(60):
                    if len(routes) >= 5:
                        return routes[:5]
                    add_route(_route_from_sequence(index, seq, budget, mode, max_distance * (1.25 if allow_over_budget else 1.0), allow_over_budget, include_bar, strict, include_terms, []))
...
with st.sidebar:
    ...
    with st.form("condition_form", clear_on_submit=False):
        ...
        max_distance = st.slider("희망 전체거리 (km)", min_value=0.5, max_value=5.0, value=2.0, step=0.5, help="코스 전체 동선이 이 거리 이내인 코스만 추천합니다. 장소 4개는 보통 1.5~2km 이상이 필요합니다.")
...
if submitted:
    ...
    api_key = get_api_key()
    if not api_key:
        st.session_state.last_error = "KAKAO_REST_API_KEY가 설정되지 않았습니다. 로컬 실행 시에는 .env 파일, Streamlit Cloud 배포 시에는 Secrets에 키를 등록하세요."
    elif not moods:
        ...
        try:
            with st.spinner("카카오 API에서 장소를 수집하고 코스를 생성하는 중입니다..."):
                ...
                routes = build_routes(df, int(budget), transport, include_bar, int(place_count), float(max_distance), bool(allow_over_budget), include_terms, moods)
                
                if not routes:
                    # 어떤 조건이 코스 생성을 막는지 진단 (이미 수집한 df 재사용 → 추가 API 호출 없음)
                    def _can_build(b, d, over):
                        try:
                            return bool(build_routes(df, int(b), transport, include_bar, int(place_count), float(d), bool(over), include_terms, moods))
                        except RuntimeError:
                            return False

                    hint = ""
                    if _can_build(budget, 5.0, allow_over_budget):
                        # 거리만 넉넉히 하면 되면 거리가 원인
                        hint = (f" '희망 전체거리'가 너무 짧습니다(현재 {max_distance:.1f}km)."
                                f" 장소 {place_count}개는 이동 구간이 {max(place_count-1,1)}번이라 보통 1.5~2km 이상이 필요합니다. 거리를 늘려보세요.")
                    elif _can_build(100000, max_distance, True):
                        # 예산만 넉넉히 하면 되면 예산이 원인
                        hint = " 예산이 빠듯합니다. 예산을 높이거나 '예산 조금 초과 가능'을 체크해보세요."
                    else:
                        if include_bar:
                            hint = " 술집 포함 시 예상비용이 크게 올라갑니다. 예산을 높이거나 '예산 조금 초과 가능'을 체크해보세요."
                        hint += " 조건을 완화하거나 지역을 넓혀보세요."
                    if include_terms:
...
if routes and meta:
    ...
    for idx, route in enumerate(shown_routes, start=1):
        ...
        budget_label = "예산 내" if route["budget_ok"] else "예산 조금 초과 코스"
        st.markdown(
            f"""
