...
def get_api_key() -> str:
    """Read Kakao REST API key from Streamlit Cloud Secrets first, then local .env."""
    try:
...
if submitted:
    ...
    api_key = get_api_key()
    if not api_key:
        st.session_state.last_error = ".env 파일에 KAKAO_REST_API_KEY가 없습니다. 현재 실행 중인 폴더 안의 .env 파일을 확인하세요."
    elif not moods:
