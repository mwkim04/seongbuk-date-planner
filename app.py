...
# Streamlit 기본 빨간 포인트 색상을 앱 시작 시점부터 핑크로 고정
# (.streamlit/config.toml이 누락되거나 캐시가 남아도 슬라이더/체크박스가 빨간색으로 뜨는 것을 방지)
os.environ["STREAMLIT_THEME_PRIMARY_COLOR"] = "#ff4f9a"
os.environ["STREAMLIT_THEME_RED_COLOR"] = "#ff4f9a"
os.environ["STREAMLIT_THEME_RED_BACKGROUND_COLOR"] = "#fff2f7"
os.environ["STREAMLIT_THEME_RED_TEXT_COLOR"] = "#ff2f86"
os.environ["STREAMLIT_THEME_SIDEBAR_PRIMARY_COLOR"] = "#ff4f9a"
...
APP_VERSION = "red_points_pink_bar_mood_fixed"
PINK = "#ff4f9a"
HOT_PINK = "#ff2f86"
LIGHT_PINK = "#fff2f7"
BORDER = "#ffd0e2"
...
st.markdown(
    f"""
<style>
:root, html, body, .stApp, [data-testid="stAppViewContainer"], section[data-testid="stSidebar"] {{
  --primary-color:{PINK} !important;
  --primary-color-rgb:255,79,154 !important;
  --st-primary-color:{PINK} !important;
  --st-primary-color-rgb:255,79,154 !important;
  --st-red-color:{PINK} !important;
  --st-red-background-color:{LIGHT_PINK} !important;
  --st-red-text-color:{HOT_PINK} !important;
}}
...
/* 빨간색으로 남는 Streamlit 기본 포인트만 핑크로 변경 */
div[data-testid="stSlider"] [data-baseweb="slider"] [role="slider"] {{
  background-color:{PINK} !important;
  border-color:{PINK} !important;
  box-shadow:0 0 0 0.2rem rgba(255,79,154,.18) !important;
}}
...
div[data-testid="stSlider"] [data-baseweb="slider"] [style*="#FF4B4B"],
div[data-testid="stCheckbox"] [style*="rgb(255, 75, 75)"],
div[data-testid="stCheckbox"] [style*="rgb(255,75,75)"],
div[data-testid="stCheckbox"] [style*="#ff4b4b"],
div[data-testid="stCheckbox"] [style*="#FF4B4B"] {{
...
def mood_tags_for(name: str, category_name: str, place_type: str) -> List[str]:
    ...
    tags = set()
    if place_type == "술집":
        # 술집은 활동적이 아니라 감성/조용한 분위기로 분류
        tags.update(["감성", "조용한"])
    if place_type in ["카페", "산책", "포토부스"]:
        tags.update(["감성", "조용한"])
    if place_type in ["활동", "산책", "포토부스"]:
        tags.update(["활동적"])
    if any(x in text for x in ["한옥", "레트로", "시장", "다방"]):
        tags.update(["레트로", "감성"])
    if place_type != "술집" and any(x in text for x in ["포차", "호프", "맥주", "노래방", "볼링", "오락", "보드게임"]):
        tags.update(["활동적"])
