...
/* 최종 핑크 고정: 빨강으로 보이는 BaseWeb 기본값까지 전부 덮어쓰기 */
:root, html, body {
  --primary-color: #ff4f9a !important;
  --red-500: #ff4f9a !important;
  --red-600: #ff4f9a !important;
  --pink-main: #ff4f9a !important;
}
...
section[data-testid="stSidebar"] [role="slider"] {
  ...
  background-color: #ff4f9a !important;
  border-color: #ff4f9a !important;
  box-shadow: 0 0 0 2px rgba(255,79,154,.18) !important;
}
section[data-testid="stSidebar"] input[type="checkbox"],
...
section[data-testid="stSidebar"] div.stButton button[kind="primary"] {
  background: linear-gradient(90deg,#ff4f9a,#ff7ab8) !important;
  border-color: #ff4f9a !important;
}
...
def sequence_templates(place_count: int, include_bar: bool, budget: int) -> List[List[str]]:
    """자연스러운 범위 안에서 시작 순서까지 다양하게 만든 데이트 흐름."""
    if place_count <= 1:
        return [["식당"], ["카페"], ["산책"], ["활동"]]

    if include_bar:
        # 술집은 너무 앞에 오지 않도록 뒤쪽에 두되, 시작 장소는 다양하게 함.
        base = {
            2: [["카페", "술집"], ["산책", "술집"], ["식당", "술집"], ["활동", "술집"]],
            3: [
                ["카페", "산책", "술집"],
                ["산책", "카페", "술집"],
                ["식당", "카페", "술집"],
        ...
        # 무조건 식당→카페로 시작하지 않도록 카페/산책/활동/포토부스 시작 흐름을 충분히 넣음.
        base = {
            2: [["식당", "카페"], ["카페", "산책"], ["산책", "카페"], ["카페", "포토부스"], ["활동", "카페"], ["포토부스", "카페"]],
            3: [
                ["카페", "산책", "식당"],
                ["산책", "카페", "식당"],
                ["포토부스", "카페", "식당"],
...
def weighted_pick_from_candidates(cand: pd.DataFrame) -> Dict | None:
    ...
    if cand.empty:
        return None
    cand = cand.copy().sort_values("rank", ascending=False).head(min(22, len(cand)))
    # 상위권 안에서 너무 1등만 고르지 않도록 완만한 가중치 적용
    weights = [max(1.0, (len(cand) - idx) ** 0.7) for idx in range(len(cand))]
