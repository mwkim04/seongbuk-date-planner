...
/* 슬라이더 채워진 트랙의 기본 빨강(rgb 255,75,75 / #ff4b4b)을 핑크로 강제 (Streamlit 버전 무관) */
...
/* 트랙 채움 div를 한 번 더 직접 겨냥 (rail 회색은 유지) */
section[data-testid="stSidebar"] [data-baseweb="slider"] > div > div > div:first-child {{
  background:{PINK} !important;
}}
.stSlider [data-baseweb="slider"] [style*="rgb(255, 75, 75)"],
.stSlider [data-baseweb="slider"] [style*="rgb(255,75,75)"] {{
  background:{PINK} !important; background-color:{PINK} !important; background-image:none !important;
}}
.hero {{
    ...
    for strict in [True, False]:
        for seq in templates:
            for _ in range(80):
                if len(routes) >= 5:
                    return routes[:5]
                add_route(_route_from_sequence(index, seq, budget, mode, max_distance, allow_over_budget, include_bar, strict, include_terms))
    ...
    if len(routes) < 5 and allow_over_budget:
        for seq in templates:
            for _ in range(60):
                if len(routes) >= 5:
                    return routes[:5]
                add_route(_route_from_sequence(index, seq, budget, mode, max_distance * 1.25, True, include_bar, False, include_terms))
            ...
            with st.spinner("카카오 API에서 장소를 수집하고 코스를 생성하는 중입니다..."):
                ...
                if not routes:
                    ...
                    def _can_build(b, d, over):
                        try:
                            return bool(build_routes(df, int(b), transport, include_bar, int(place_count), float(d), bool(over), include_terms))
                        except RuntimeError:
                            return False
