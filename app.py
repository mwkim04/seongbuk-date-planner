...
_KAKAO_MAX_WORKERS = 8  # 병렬 호출 동시 처리 수 (429 회피 위해 과도하게 높이지 않음)
...
def collect_places(region: str, include_bar: bool, api_key: str, include_terms: List[str] | None = None) -> pd.DataFrame:
    ...
    region_terms = REGION_KEYWORDS.get(region, [region])[:3]
    ...
    for term in include_terms:
        for region_term in REGION_KEYWORDS.get(region, [region])[:3]:
