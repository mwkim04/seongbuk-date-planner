...
## 🔎 실데이터 품질 진단 (선택)
실제 카카오 데이터에 이상한 주소/장소가 섞여 추천되지 않는지 점검하려면:
```bash
python diagnose_data.py                       # 전체 지역 점검
python diagnose_data.py --region 안암동        # 특정 지역만
python diagnose_data.py --report quality.html  # HTML 리포트로 저장
```
주소 유효성, 좌표의 지역 범위, 데이트 부적합 업종 잔존, 술집 누출, 링크/비용 이상을 자동 검사합니다. 본인 API 키(.env)가 필요하며 호출 한도를 사용합니다.
